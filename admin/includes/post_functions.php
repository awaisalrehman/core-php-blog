...
function createPost($request_values)
{
	...
	if (!empty($featured_image)) {
		...
		$target = "../../static/uploads/post_featured_images/" . basename($featured_image);
	...
	* * * * * * * * * * * * * * * * * * * * * */
	...
	{
		global $conn, $errors, $title, $post_slug, $body, $published, $isEditingPost, $post_id;
		
		if (isset($_SESSION['errors'])) {
			$errors = $_SESSION['errors'];
			$title = $_SESSION['post']['title'];
			$body = $_SESSION['post']['body'];
			$published = $_SESSION['post']['published'];
	...
	function updatePost($request_values)
	{
		...
		if (count($errors) == 0) {
			...
			$_SESSION['post'] = ['title'=> $title, 'body'=> $body, 'published'=> $published];
			header('location: /admin/posts/create.php?edit-post=' . $post_id);
		}
	}
	// delete blog post
